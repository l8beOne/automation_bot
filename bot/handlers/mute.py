...
async def mute(
    ...
    """
    Этот хэндлер будет срабатывать на команду /mute
    и принимает вместе с ней аргументы: длительность и причина мьюта.
    """
    ...
    try:
        ...
        await message.answer(
            "Для мьюта пользователя введите кол-во времени на которое вы "
            "хотите запретить ему писать сообщения в чат с ботом. "
            "Также укажите причину мьюта.\n"
    ...
    await bot.send_message(
        user_id,
        f"Сотрудник учебного офиса запретил вам писать сообщения в бота "
        f"на время {mute_time_count} {mute_time_type}.\n"
...
async def unmute_process(bot: Bot, userstatus: UserStatusClass, user_id: int):
    """
    Этот хэндлер будет срабатывать через время,
    указанное в аргуменатах к команде /mute.
    """
    await userstatus.update_user_status("user_status", user_id, "active")
    await bot.send_message(
        user_id,
        "Время мьюта закончилось, вы снова можете писать сообщения в бота"
    )
